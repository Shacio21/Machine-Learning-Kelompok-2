...
class PredictRequest(BaseModel):
    ...
    stress_level: int = Field(..., ge=1, le=10, description="Tingkat stres (1-10)")
    daily_steps: int = Field(..., ge=0, description="Jumlah langkah harian")
    ...
    blood_pressure_systolic: int = Field(..., ge=80, le=200, description="Tekanan darah sistolik")
    blood_pressure_diastolic: int = Field(..., ge=50, le=130, description="Tekanan darah diastolik")
...
class PredictResponse(BaseModel):
    prediction: str = Field(..., description="Prediksi tingkat fitness")
